...
@property (nonatomic, weak) NSURLSessionDataTask* dataSession;
@property (nonatomic, strong) NSURLSession* session;
@property (nonatomic, strong) NSMutableDictionary* responsesData;
@property (nonatomic, strong) NSMutableDictionary* taskIdsToCommand;
@end
...
- (void)pluginInitialize {
	...
	NSURLSessionConfiguration *backgroundConfiguration = [NSURLSessionConfiguration backgroundSessionConfigurationWithIdentifier:@"localhost"];
	self.session = [NSURLSession sessionWithConfiguration:backgroundConfiguration delegate:self delegateQueue:[NSOperationQueue mainQueue]];
	self.taskIdsToCommand = [NSMutableDictionary dictionary];
}
...
- (void)uploadData:(NSData*)fileData command:(CDVInvokedUrlCommand*)command
{
	...
	NSURLSessionDataTask* dataSession = [self.session dataTaskWithRequest:req];
	self.taskIdsToCommand[@(dataSession.taskIdentifier)] = command;
	[dataSession resume];
}
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
	...
	CDVInvokedUrlCommand* command = self.taskIdsToCommand[@(task.taskIdentifier)];
	NSMutableData *responseData = self.responsesData[@(task.taskIdentifier)];
	
	if (responseData) {
		
		NSString* uploadResponse = [[NSString alloc] initWithData:responseData encoding:NSUTF8StringEncoding];
		if (uploadResponse == nil) {
			uploadResponse = [[NSString alloc] initWithData:responseData encoding:NSISOLatin1StringEncoding];
		}
		...
		if (uploadResponse != nil) {
			...
			[uploadResult setObject:[httpResponse allHeaderFields] forKey:@"headers"];
		}
		[uploadResult setObject:[NSNumber numberWithLongLong:task.countOfBytesSent] forKey:@"bytesSent"];
		[uploadResult setObject:[NSNumber numberWithInteger:httpResponse.statusCode] forKey:@"responseCode"];
		...
		result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsDictionary:[self createFileTransferError:CONNECTION_ERR AndSource:source AndTarget:target]];
	}
